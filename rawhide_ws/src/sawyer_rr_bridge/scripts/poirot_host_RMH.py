...
property int32 OpenGripperButton
property int32 CloseGripperButton
property int32 Has_Collided
property int32 gripperPos
property int32 PB1
property int32 PB2
property int32 PB3
property int32 PB4
...
function void setGRIPpos( int32 cmd1)
function void setHEADangle(int32 cmd2)


function void printGripperCommand()
function void printGripperStatus()
...
class Sawyer_impl(object):
    def __init__(self):
        ...
        rospy.Subscriber("/robot/digital_io/right_lower_button/state", DigitalIOState,self.GripperCloseCallback)
        rospy.Subscriber("/robot/digital_io/right_upper_button/state", DigitalIOState,self.GripperOpenCallback)
        ...
        rospy.Subscriber("Robotiq2FGripperRobotInput", inputMsg.Robotiq2FGripper_robot_input, self.GripperStatusCallback)
        self.gripperpub=rospy.Publisher('Robotiq2FGripperRobotOutput', outputMsg.Robotiq2FGripper_robot_output)
    ...
    @property
    def CloseGripperButton(self):

        return self.closegripperbutton

    @property
    def OpenGripperButton(self):

        return self.opengripperbutton
    ...
    def update_hascollided(self):
        self.hascollided=self._right.has_collided()
    ...
    def GripperStatusCallback(self,data):

        self.gripperstatus.gACT=data.gACT
        self.gripperstatus.gGTO=data.gGTO
        self.gripperstatus.gSTA=data.gSTA
        self.gripperstatus.gOBJ=data.gOBJ
        self.gripperstatus.gFLT=data.gFLT
        self.gripperstatus.gPR=data.gPR
        self.gripperstatus.gPO=data.gPO
        self.gripperstatus.gCU=data.gCU

    def printGripperCommand(self):
        return self.grippercommand

    def printGripperStatus(self):
        return self.gripperstatus

    def get_gripper_pos_command(self,griper_pos):

        self.des_gripper_pos=griper_pos

    def set_gripper_command(self):
        #print(self.closegripperbutton,self.opengripperbutton)
        #print(self.grip_des_pos)
        if self.closegripperbutton == 1:
            self.grippercommand.rACT = 1
            self.grippercommand.rGTO = 1
            self.grippercommand.rSP  = 255
            self.grippercommand.rFR  = 150
            self.grippercommand.rPR = 255
            self.grip_des_pos= 444

        if self.opengripperbutton==1:
            self.grippercommand.rACT = 1
            self.grippercommand.rGTO = 1
            self.grippercommand.rSP  = 255
            self.grippercommand.rFR  = 150
            self.grippercommand.rPR = 0
            self.grip_des_pos= 444

        elif self.grip_des_pos!= 444:
            self.grippercommand.rACT = 1
            self.grippercommand.rGTO = 1
            self.grippercommand.rSP  = 255
            self.grippercommand.rFR  = 150
            self.grippercommand.rPR = self.grip_des_pos



    def setGRIPpos(self,cmd):
        self.grip_des_pos=cmd

    def GripperCloseCallback(self,data):
        #roslaunch spacenav_node classic.launch


        #I think these have to be self. variables due to the nature of callback. data is only accessed in the callback

        self.closegripperbutton=data.state

    def GripperOpenCallback(self,data):
        #roslaunch spacenav_node classic.launch


        #I think these have to be self. variables due to the nature of callback. data is only accessed in the callback

        self.opengripperbutton=data.state
    ...
    def pushed1callback(self,data):
        ...
        if data.data==True:
	       ...
	       self.pb1=0
        #print(self.pb1)
    def pushed2callback(self,data):
        if data.data==True:
            self.pb2=1
        else:
	       self.pb2=0
    def pushed3callback(self,data):
        if data.data==True:
            self.pb3=1
        else:
	       self.pb3=0
    def pushed4callback(self,data):
        if data.data==True:
            self.pb4=1
        else:
	       self.pb4=0

    def setHEADangle(self,cmd2):
        self.headangle=cmd2
    ...
    def display_worker(self):
        while self._running:
            self.head_display.display_image(self.image,display_rate=1)
            self.head.set_pan(self.headangle)

            t1 = time.time()

            while (time.time() - t1 < .1):#self.RMH_delay):
